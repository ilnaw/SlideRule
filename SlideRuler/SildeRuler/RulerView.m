...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [[STShakeManager manager] initHapitcEngine];
...
//        self.rulerCollectionView.automaticallyAdjustsScrollIndicatorInsets = NO;
//    } else {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [[STShakeManager manager] doCHHapticTransientWithKeyTime:index];
    //保证数据在范围内
...
//        });
        [[STShakeManager manager] doCHHapticTransientHeavy];
        //等视图更新完成才选中
        dispatch_async(dispatch_get_main_queue(), ^{
            [self selectCell];
        });
