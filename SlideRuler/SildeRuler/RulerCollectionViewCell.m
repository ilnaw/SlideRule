...
- (void)makeCellSelect {
    ...
    if (kHorizontalCell) {
        ...
        self.selectTextLayer.frame = CGRectMake((CGRectGetWidth(self.contentView.frame) - (size.width + 1))/2.0, startY, (size.width+1), size.height);
    } else {
        //垂直方向
        CGFloat startX = 0;
        if (kNumberLeft) {
        ...
        self.selectTextLayer.frame = CGRectMake(startX, (CGRectGetHeight(self.contentView.frame) - size.height)/2.0, size.width+4, size.height);
    }
    
    self.selectTextLayer.string = [[NSAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName: self.rulerConfig.selectFont == nil ? [UIFont systemFontOfSize:18]:self.rulerConfig.selectFont, NSForegroundColorAttributeName: self.rulerConfig.selectColor == nil ? UIColor.grayColor :self.rulerConfig.selectColor}];
